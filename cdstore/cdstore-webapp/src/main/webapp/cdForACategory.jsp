		...
		<div class="row">
			<div id="sidebar" class="span3">
				<ul class="nav nav-list bs-docs-sidenav" style="display: block;">
					<li class="subMenu"><a><strong>Browse Categories</strong></a></li>
					<c:forEach var="category" items="${categories}">
						<c:choose>
							<c:when test="${selectedCategory == category}">
								<li class="active"><a
									href="cdsForCategories?category=${category}"><c:out
											value="${category}"></c:out></a></li>
							</c:when>
							<c:otherwise>
								<li><a href="cdsForCategories?category=${category}"><c:out
											value="${category}"></c:out></a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
			...
			<div class="span9">
				<div>
					<h3>
						All Products in
						<c:out value="${selectedCategory}"></c:out>
						category
					</h3>
					<ul class="thumbnails">
						<c:forEach var="cd" items="${cdForCategoryList}">
							<li class="span3">
								<div class="thumbnail">
									<img
