	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException,
		...
		if (confirmOrderButton != null) {
			...
			if (checkedIds != null && checkedIds.size() > 0) {
				...
				String creditCardNumber = request.getParameter("inputCreditTextBox");
				request.getSession().setAttribute("cardNumber", creditCardNumber);
				String expiryDate = request.getParameter("expiryDateTextBox");
				...
				String responseString = purchaseOrderService.purchase(poOrder);
				if (responseString.equals("success")) {
					// order was placed successfully, take the user to home page
					// user can see account on home page, which shows the order
					// history
					// But we need to clean the shopping cart from session first
					request.getSession().setAttribute("selectedcds", null);
					this.getServletContext().getRequestDispatcher("/orderSucess.jsp").include(request, response);
					return;
				} else if (responseString.equals("creditcarderror")) {
					// every fifth request is rejected with credit card error,
					// in such case redirect user to confirm order page where
					// user needs to fill out the credit card information again.
					// of-course, a warning message will be displayed that
					// provided credit card information was wrong
					this.getServletContext().getRequestDispatcher("/creditCardError.jsp").forward(request, response);
					return;
				} else if (responseString.equals("tryagain")) {
					// some exception occurred while saving the order, redirect
					// the user to home page without clearing the cart detail
					// from session
			...
			// if user clicks the cancel button then clear the cart and redirect
