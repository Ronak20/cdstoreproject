...
public class CDShowServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			// and then comes back to homepage(this page) again. in such a case
			// we
			...
			ArrayList<String> interimObj = (ArrayList<String>) request
					.getSession().getAttribute("selectedcds");
			if (interimObj != null && interimObj.size() > 0) {
				List<CD> selectedCds;

				selectedCds = iCdService.getCds(interimObj);
