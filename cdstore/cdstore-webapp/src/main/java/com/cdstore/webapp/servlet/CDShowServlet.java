...
public class CDShowServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		//look if user had already selected some cds. This may happen in a case when user goes to Cds by categories web-page
		//and then comes back to homepage(this page) again. in such a case we need to find out the selected cds and display the updated cart 
		...
		ArrayList<String>interimObj =(ArrayList<String>) request.getSession().getAttribute("selectedcds");
			if(interimObj !=null){
				...
				request.getSession().setAttribute("nocdselected","false" );	//set this to false so that user doesn't see warning message.
			}
